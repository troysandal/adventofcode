...
runProgram({
    ...
    input: () => {
        ...
        //.sort((l,r) => l.value.pixel.charCodeAt(0) - r.value.pixel.charCodeAt(0))
    ...
    output: (v) => {
        if (v === STATUS_WALL) {
            map.setPt(testPos, {pixel:'#', count:0})
        } else if (v === STATUS_OK) {
            let x = map.getPt(pos) || {pixel:'.', count:0}
            map.setPt(pos, {pixel:'.', count:x.count + 1})
            pos = testPos
            x = map.getPt(pos) || {pixel:'D', count:0}
            map.setPt(testPos, {pixel:'D', count:x.count + 1})
        } else {
            let x = map.getPt(pos) || {pixel:'.', count:0}
            map.setPt(pos, {pixel:'.', count:x.count + 1})
            pos = testPos
            console.log(`Found Oxygen at ${pos.x}, ${pos.y}`)
            const {offset, grid} = bitmapTo2D(map)

            var graph = new AStar.Graph(grid);
            var start = graph.grid[0 - offset.y][0 - offset.x];
            var end = graph.grid[pos.y - offset.y][pos.x - offset.x];
            var astar = AStar.astar.search(graph, start, end);
            console.log(`Min steps is ${astar.length}`)
            debugger
        }
        console.log('')
        // map.print()
