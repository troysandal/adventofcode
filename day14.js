...
function isTerminal(ingredient) {
    return (ingredient.recipe.length === 1) &&
           !ingredient.recipe[0].node // ORE is undefined
  }

  // basically a breadth first walk until we hit all the leaf nodes
  function recipeFor(map, root, qty, recipe = {}) {
      qty = Math.ceil(qty / root.qty)
      for (let ingredient of root.recipe) {
          if (isTerminal(ingredient.node)) {
              recipe[ingredient.node.name] = (recipe[ingredient.node.name] || 0) + (ingredient.qty * qty)
          } else {
              recipeFor(map, ingredient.node, ingredient.qty * qty, recipe)
          }
      }
      return recipe
  }
  ...
  function fuelForRecipe(map, recipe) {
    ...
    for (let name of Object.keys(recipe)) {
        console.assert(isTerminal(map[name]))
